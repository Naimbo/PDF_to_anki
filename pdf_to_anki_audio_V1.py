...
def _wav_to_mp3(wav_path: Path, out_mp3_path: Path) -> bool:
    result = subprocess.run(
        ["ffmpeg", "-y", "-loglevel", "error", "-i", str(wav_path),
         "-codec:a", "libmp3lame", "-b:a", "48k", str(out_mp3_path)],
        capture_output=True, text=True,
    )
    return result.returncode == 0 and out_mp3_path.exists()


def synthesize_word_audio_rhvoice(word: str, out_mp3_path: Path, voice: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ["RHVoice-test", "-p", voice, "-i", str(txt_path), "-o", str(wav_path)],
            capture_output=True, text=True, timeout=20,
        )
        if result.returncode != 0 or not wav_path.exists() or wav_path.stat().st_size == 0:
            return False
        return _wav_to_mp3(wav_path, out_mp3_path)
...
def synthesize_word_audio_espeak(word: str, out_mp3_path: Path, voice: str, speed: int) -> bool:
    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp_wav:
        wav_path = Path(tmp_wav.name)
    try:
        result = subprocess.run(
            ["espeak-ng", "-v", voice, "-s", str(speed), "-w", str(wav_path), word],
            capture_output=True, text=True,
        )
        if result.returncode != 0 or not wav_path.exists() or wav_path.stat().st_size == 0:
            return False
...
def add_audio_to_entries(entries: list[dict], media_dir: Path, engine: str, voice: str, speed: int) -> list[str]:
    """
    Generates one mp3 per entry into media_dir, and sets entry["audio_tag"]
    accordingly. Returns the list of media file paths to bundle into the
    .apkg. Entries where synthesis failed get an empty audio_tag so the deck
    still builds (that word just won't have a sound clip).
    """
    ...
    for e in entries:
        filename = f"ru2000_{e['rank']}.mp3"
        out_path = media_dir / filename
        if engine == "rhvoice":
            ok = synthesize_word_audio_rhvoice(e["word"], out_path, voice=voice)
        else:
            ok = synthesize_word_audio_espeak(e["word"], out_path, voice=voice, speed=speed)
        if ok:
            e["audio_tag"] = f"[sound:{filename}]"
            media_files.append(str(out_path))
        else:
            e["audio_tag"] = ""
            failed.append(e["rank"])

    if failed:
        print(f"Warning: audio generation failed for {len(failed)} entries "
              f"(ranks: {', '.join(failed[:10])}{'...' if len(failed) > 10 else ''})")
...
MODEL_ID = 1607392320  # different from the no-audio script's model id (extra field)
DECK_ID = 2059852317
...
def write_csv(entries: list[dict], path: str) -> None:
    with open(path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f, fieldnames=["rank", "word", "pronunciation", "translation", "example_ru", "example_en", "audio_tag"]
        )
        writer.writeheader()
        for e in entries:
            writer.writerow(e)
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        if not args.no_audio:
            print(f"Generating pronunciation audio for {len(entries)} words ...")
            default_voice = "Anna" if engine == "rhvoice" else "ru"
            voice = args.voice or default_voice
            media_files = add_audio_to_entries(entries, Path(tmp_dir), engine=engine, voice=voice, speed=args.tts_speed)
