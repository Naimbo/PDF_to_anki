...
def assign_sequence(entries: list[dict]) -> list[str]:
    """
    Assigns entry["seq"] = 1, 2, 3... in PDF appearance order (which is the
    true frequency order, most to least common). --start/--end and --limit
    filter on this, NOT on the printed "rank" text, so a range like
    --start 1 --end 500 is guaranteed to be the actual top 500 words even if
    the rank numbers printed in the PDF ever have a gap, duplicate, or a
    parsing hiccup. Returns a list of ranks where seq and the parsed rank
    disagree, so you can spot-check the source PDF around those entries.
    """
    mismatches = []
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        metavar="N",
        default=None,
        help="Only include the words from position N onward (1-based, "
             "inclusive, by TRUE frequency order - i.e. the Nth most "
             "frequent word, regardless of what number is printed next to "
             "it in the PDF). Combine with --end for a range, "
             "e.g. --start 501 --end 1000.",
    )
    parser.add_argument(
        "--end",
        type=int,
        metavar="N",
        default=None,
        help="Only include words up to position N (inclusive, by true "
             "frequency order). Combine with --start for a range, "
             "e.g. --start 501 --end 1000.",
    )
    parser.add_argument(
        "-o",
...
def main() -> None:
    ...
    if rank_mismatches:
        print(
            "Warning: printed rank number didn't match actual position in "
            "the PDF for these entries (using true position, not the "
            "printed number, for --start/--end/--limit): "
    ...
    if args.start is not None or args.end is not None:
        ...
        hi = args.end if args.end is not None else total_entries
        entries = [e for e in entries if lo <= e["seq"] <= hi]
        print(
            f"Using positions {lo}-{hi} (true frequency order): {len(entries)} of "
            f"{total_entries} entries ({len(entries) * 2} cards)."
        )
        if args.deck_name == "Russian Most Frequent Words":
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        if args.no_audio:
            ...
            print(
                f"  word: {n_word}/{len(entries)}   "
                f"sentence: {n_sentence}/{len(entries)}   "
                f"sentence (slow): {n_slow}/{len(entries)}"
            )
