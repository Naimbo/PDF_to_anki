...
def parse_entries(raw_text: str, debug: bool = False) -> list[dict]:
    ...
    for raw_line in raw_text.splitlines():
        ...
        # A translation is still "not finished" in two distinct situations:
        #
        # 1. It has an unclosed '(' - e.g. "The same (in combination with
        #    'такой', 'тот'," then "'то')" on the next line. HEADER_RE only
        #    matched the first line (nothing requires the header to be
        #    "finished" before end-of-line).
        #
        # 2. This document's format is always Russian example -> English
        #    example, in that order. So an English-dominant line showing up
        #    BEFORE we've captured any Russian example yet for this entry
        #    cannot legitimately be ExampleEN - the only thing it can be is
        #    more of a long definition that wrapped across several lines,
        #    e.g. "A possessive pronoun common for all" / "personal pronouns
        #    (my, his, her, its, our, your, their) and refers" / "to thе
        #    subject of the sentence. This form is for neuter nouns." - note
        #    this case has NO unclosed parenthesis at all, which is why rule
        #    1 alone doesn't catch it; the signal here is ORDER, not parens.
        #
        # Either signal alone is enough to treat the line as more
        # translation rather than routing it to an example field.
        translation_unbalanced = current["translation"].count("(") > current["translation"].count(")")
        ...
        if translation_unbalanced:
            ...
            if debug:
                ...
                print("Detected: TRANSLATION CONTINUATION (closing an open parenthesis)")
                print(f"  Parsed translation: {current['translation']}\n")
            continue

        # Next: is this ONE line that actually contains BOTH a Russian
        # example and its English translation side by side (see
        # split_bilingual_line)? This check must come BEFORE the
        # order-based "translation_still_incomplete" rule below: a merged
        # bilingual line can have as many Latin letters as Cyrillic ones,
        # which would otherwise look identical to "more English definition
        # text" - but a successful content-aware split is much stronger,
        # more specific evidence that this is genuinely the RU/EN example
        # pair, not leftover translation.
        ...
        if translation_still_incomplete:
            ...
            if debug:
                print(f"Raw extracted line:\n  {line}")
                print("Detected: TRANSLATION CONTINUATION (no Russian example captured yet for this entry)")
                print(f"  Parsed translation: {current['translation']}\n")
            continue
